...
export default function MissingReportsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["reports", "missing", periodType, date],
    queryFn: () => reportsApi.missing({ period_type: periodType, period_start: date, period_end: date }),
  });
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="mt-1 text-sm text-secondary">Districts n&apos;ayant pas soumis pour la période sélectionnée.</p>
      </div>
      ...
      <Card>
        {isLoading ? (
          ...
          <EmptyState icon={CheckCircle2} title="Tous les districts ont soumis" description="Aucun rapport manquant pour cette période." />
        ) : (
          <Table>
            <Thead>
              <tr>
